...
public class MainActivity extends AppCompatActivity implements View.OnClickListener {
            ...
            for(ScanResult result : scanResults) {
                try {
                    while (scanResultsSize >= 0) {
                        ...
                        MyItem newItem = new MyItem(scanResults.get(scanResultsSize).SSID, scanResults.get(scanResultsSize).capabilities, String.valueOf(scanResults.get(scanResultsSize).level) + " dBm", String.valueOf(scanResults.get(scanResultsSize).frequency) + " MHz", String.valueOf(df.format(distance)) + " m", scanResults.get(scanResultsSize).BSSID, String.valueOf(scanResults.get(scanResultsSize).operatorFriendlyName), String.valueOf(scanResults.get(scanResultsSize).venueName), String.valueOf(scanResults.get(scanResultsSize).describeContents()), String.valueOf(scanResults.get(scanResultsSize).is80211mcResponder()), String.valueOf(scanResults.get(scanResultsSize).isPasspointNetwork()), images[sLevel]);
    ...
    class MyItemLongClicked implements AdapterView.OnItemLongClickListener{
        ...
        public boolean onItemLongClick(AdapterView<?> parent, View view, int position, long id) {
            ...
            intent.putExtra("bssid", list.bssid);
            intent.putExtra("operatorFriendlyName", list.OperatorFriendlyName);
            intent.putExtra("venueName", list.VenueName);
            intent.putExtra("Contents", list.Contents);
            intent.putExtra("mcResponder", list.mcResponder);
            intent.putExtra("passPointNetwork", list.passPointNetwork);
            startActivity(intent);
            return false;
        }
    }
